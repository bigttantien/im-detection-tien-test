package io.benkon.sample.image_detection.yolo;
...
import io.benkon.sample.image_detection.Constant;


public class Yolov5TFLiteDetector {

    private final Size INPNUT_SIZE = Constant.INSTANCE.getINPUT_SIZE$app_debug();
    private final int[] OUTPUT_SIZE = Constant.INSTANCE.getOUTPUT_SIZE$app_debug();
    private final float DETECT_THRESHOLD =Constant.DETECT_THRESHOLD;
    private final float IOU_THRESHOLD = Constant.IOU_THRESHOLD;
    private final float IOU_CLASS_DUPLICATED_THRESHOLD = Constant.IOU_CLASS_DUPLICATED_THRESHOLD;


    private Boolean IS_INT8 = false;
    private final String MODEL_YOLOV5S = "yolov5s_fp16_person.tflite";
...
//    private final String MODEL_YOLOV5S = "yolov5s-dynamic.tflite";
    ...
    private String MODEL_FILE = Constant.MODEL_FILE + ".tflite";
     ...
     * @return
     */
    public RecognitionResult detect(Bitmap bitmap) {
        // Detection input shape is square so we need add padding for short edge.
        int maxDimension = Math.max(bitmap.getHeight(), bitmap.getWidth());
        float scaleX = maxDimension * 1f / INPNUT_SIZE.getWidth();
        float scaleY = maxDimension * 1f / INPNUT_SIZE.getHeight();
        int xPaddingTotal = maxDimension - bitmap.getWidth();
        int yPaddingTotal = maxDimension - bitmap.getHeight();
        ...
        if (IS_INT8) {
            imageProcessor =
                    new ImageProcessor.Builder()
                            .add(new ResizeOp(INPNUT_SIZE.getHeight(), INPNUT_SIZE.getWidth(), ResizeOp.ResizeMethod.NEAREST_NEIGHBOR))
                            .add(new NormalizeOp(1, 255))
            ...
            imageProcessor =
                    new ImageProcessor.Builder()
                            ...
                            .add(new ResizeOp(INPNUT_SIZE.getHeight(),
                                    INPNUT_SIZE.getWidth(),
                                    ResizeOp.ResizeMethod.NEAREST_NEIGHBOR))
//                            .add(new Rot90Op(4))
                            .add(new NormalizeOp(1f, 127.5f))
                            .build();
        ...
        Bitmap processedBitmap = yolov5sTfliteInput.getBitmap().copy(yolov5sTfliteInput.getBitmap().getConfig(), false);
        ...
        for (int row = 0; row < 8400; row++) {
            float x = recognitionArray[row];
            float y = recognitionArray[row + 8400];
            float w = recognitionArray[row + 2 * 8400];
            float h = recognitionArray[row + 3 * 8400];
            ...
            float confidence = recognitionArray[row + 4 * 8400];
            int labelId = 0;
            float maxLabelScores = 1.f;
...
//            }
            ...
            if (labelId == 0) {
                Recognition r = new Recognition(
                        ...
                        maxLabelScores,
                        confidence,
                        new RectF((xmin * scaleX) - (xPaddingTotal / 2f),
                                (ymin * scaleY) - (yPaddingTotal / 2f),
                                (xmax * scaleX) - (xPaddingTotal / 2f),
                                (ymax * scaleY) - (yPaddingTotal / 2f)),
                        recognitionArray);
...
//        Log.i("tfliteSupport", "recognize data size: "+allRecognitions.size());
        ...
        return new RecognitionResult(nmsFilterBoxDuplicationRecognitions, processedBitmap);
    }
...
//                    Log.i("tfliteSupport", allRecognitions.get(j).toString());
    ...
    protected ArrayList<Recognition> nmsAllClass(ArrayList<Recognition> allRecognitions) {
        ...
        Recognition[] results = new Recognition[pq.size()];
        pq.toArray(results);
//        Log.e("TAG", "Count="+results.length);
        for (int i = 0; i < results.length;i++){
            RectF loc = results[i].getLocation();
//            Log.e("TAG", i + ":"+loc.top + " " + loc.left + " " + loc.width() + " " + loc.height() + " " + loc.centerX() + " " + loc.centerY());
        }
...
//            nnApiDelegate = new NnApiDelegate(nnApiOptions);
    ...
    public void cancel() {
        tflite.setCancelled(true);
    }

    public static class RecognitionResult {
        public ArrayList<Recognition> results;
        public Bitmap processedBitmap;

        public RecognitionResult(ArrayList<Recognition> results, Bitmap processedBitmap) {
